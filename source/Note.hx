...
class Note extends FlxSprite
{
	...
	public function new(strumTime:Float, noteData:Int, ?prevNote:Note, ?sustainNote:Bool = false, ?inCharter:Bool = false, ?isAlt:Bool = false, ?bet:Float = 0, ?isPlayer:Bool = false)
	{
		super();
			...
			switch (noteTypeCheck)
			{
				...
				default:
					if (isPlayer)
						...
						frames = Paths.getSparrowAtlas("EXE_notes");
					for (i in 0...4)
					{
						animation.addByPrefix(dataColor[i] + 'Scroll', dataColor[i] + ' alone'); // Normal notes
						animation.addByPrefix(dataColor[i] + 'hold', dataColor[i] + ' hold'); // Hold
