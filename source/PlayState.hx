	...
	public function generateSong(dataPath:String):Void
	{
		...
		for (section in noteData)
		{
			...
			for (songNotes in section.sectionNotes)
			{
				...
				if (gottaHitNote)
					swagNote = new Note(daStrumTime, daNoteData, oldNote,false,false,false,songNotes[4], true);
				else
					swagNote = new Note(daStrumTime, daNoteData, oldNote,false,false,false,songNotes[4]);
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			switch (noteTypeCheck)
			{
				...
				default:
					if (player == 0)
						babyArrow.frames = Paths.getSparrowAtlas('EXE_notes');
					else
